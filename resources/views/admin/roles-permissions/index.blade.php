...
@section('breadcrumb_current', 'Roles & Permissions')
  ...
  <div class="card dashboard-hero mb-4">
    <div class="card-body p-4 p-lg-5 position-relative">
      <div class="row align-items-center g-4">
        <div class="col-lg-8">
          <span class="badge rounded-pill text-bg-light text-primary mb-3">Spatie permission manager</span>
          <h1 class="h2 fw-bold mb-3">Manage admin roles and permissions for {{ $siteName }}.</h1>
          <p class="hero-meta mb-0">
            Create permissions, group them into roles, and assign those roles to admin accounts using the `admin` guard.
          </p>
        </div>
        <div class="col-lg-4">
          <div class="info-grid">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('admin.roles-permissions.permissions.store') }}">
            ...
            <div class="mb-3">
              ...
              <input id="permission_name" name="permission_name" type="text" class="form-control @error('permission_name') is-invalid @enderror" value="{{ old('permission_name') }}" placeholder="example.manage" required />
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('admin.roles-permissions.roles.store') }}">
            ...
            <div class="row g-2">
              ...
              @empty
                <div class="col-12">
                  <div class="alert alert-light mb-0">Create at least one permission first.</div>
                </div>
              @endforelse
            </div>
      ...
      <div class="card card-outline card-secondary">
        ...
        <div class="card-body">
          @forelse ($permissions as $permission)
            <span class="badge text-bg-light border me-1 mb-1">{{ $permission->name }}</span>
          @empty
            <div class="text-secondary">No permissions created yet.</div>
          @endforelse
        </div>
      </div>
    </div>
